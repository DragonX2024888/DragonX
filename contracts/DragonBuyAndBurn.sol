...
contract DragonBuyAndBurn is Ownable, ReentrancyGuard {
    ...
    /**
     * Current DragonX Price in TitanX
     * @notice Retrieves the current price of one DragonX token in terms of TitanX tokens from Uniswap V3.
     * @return price The current price of one DragonX token in TitanX.
     * @dev This function computes the price of DragonX in TitanX using the Uniswap V3 pool.
     *      Steps to compute the price:
     *        1. Compute the pool address with the PoolAddress library using the Uniswap factory address,
     ...
     *           the square root of the price of one token in terms of the other, in 96-bit fixed-point format.
     *        4. Calculate the price by squaring sqrtPriceX96 to get the actual price ratio
     *           and adjust to TitanX's decimal format (assumed to be 18 decimals).
     *        5. Invert the price calculation if DRAGONX_ADDRESS is less than TITANX_ADDRESS. This step
     *           is necessary to ensure the price is expressed in terms of TitanX for one DragonX.
     *      Utilizes the Math.mulDiv utility for safe multiplication and division operations.
     *      Note: The price inversion is based on token order in the Uniswap pool. If DragonX is `token0`,
     *      the price is inverted to express DragonX's price in TitanX.
     */
        ...
        returns (uint256 price)
    {
        ...
        (uint256 sqrtPriceX96, , , , , , ) = pool.slot0();
        uint256 numerator1 = sqrtPriceX96 * sqrtPriceX96;
        uint256 numerator2 = 10 ** 18; // Assuming TitanX has 18 decimals, adjust if necessary
        price = Math.mulDiv(numerator1, numerator2, 1 << 192);
